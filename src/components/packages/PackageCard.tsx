  ...
  return (
    <article className="group flex flex-col justify-between rounded-2xl border border-slate-100 bg-white p-5 text-center shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-amber-200 hover:shadow-xl">
      ...
      <div className="mt-4 border-t border-slate-100 pt-3">
        <div>
          <span className="text-lg font-black text-amber-600">₹{pkg.price}</span>
          <span className="block text-[10px] font-bold uppercase tracking-wider text-slate-400">Onwards</span>
        </div>

        <button className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl border border-amber-300/80 bg-amber-50/60 py-2 text-xs font-bold text-amber-700 transition-all group-hover:bg-[#FFB800] group-hover:text-slate-950">
          <span>{pkg.linkText || 'Book Now'}</span>
